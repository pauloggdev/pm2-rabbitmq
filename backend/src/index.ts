...
app.get('/getAllStudents', async function (req: any, res: any) {
    ...
    const studentsFromCache = await client.get('getStudents');
    if (studentsFromCache) {
        console.log('Retrieved students from cache');
        res.json(JSON.parse(studentsFromCache));
        return;
    }
    ...
    console.log('Retrieved students from database');
    //await client.set('getProducts', JSON.stringify(data), { EX: 10 });
    await client.set('getStudents', JSON.stringify(data));
    res.json(data);
})
app.get('/salve', async (req, res) => {
    await client.del('getStudents')
    res.json({
        ok: 1
    })
    return;
